...
//useEffect 2
  useEffect(() => {
    fetchZonesData();
  }, [editMode, selectedZone, selectedDate, dateDebut, dateFin]);
  ...
  useEffect(() => {
    ...
    const loadZoneDetails = async () => {
      ...
      if (selectedZone >= 0 && selectedZone < zones.length) {
        const zoneId = zones[selectedZone]._id;
  ...
  useEffect(() => {
    if (selectedZone && zones.length > 0) {
      ...
      setSelectedZone(zoneIndex !== -1 ? zoneIndex : 0);
  ...
  const handleNbBenevoleChange = (index, value) => {
    ...
    updatedZones[selectedZone].horaireCota[index].nb_benevole = value;
  ...
  const handleSaveChanges = async () => {
    console.log("nom zone modifié", currentZoneDetails.nom_zone_plan || currentZoneDetails.nom_zone_benevole);
    ...
    const url = `http://localhost:3500/${isZoneBenevole() ? "zone" : "zonePlan"}/${zoneToUpdate._id}`;
...
export default AncienDisplayZone;